...
public class NearbyLightsBlocksSensor<E extends LivingEntity> extends PredicateSensor<BlockState, E> {
	private static final List<MemoryModuleType<?>> MEMORIES = ObjectArrayList.of(AftershockMemoryTypes.NEARBY_LIGHT_BLOCKS.get());
	...
	public SensorType<? extends ExtendedSensor<?>> type() {
		return AftershockSensors.NEARBY_LIGHT_BLOCKS.get();
	}
	...
	protected void doTick(ServerLevel level, E entity) {
		...
		if (blocks.isEmpty())
			BrainUtils.clearMemory(entity, AftershockMemoryTypes.NEARBY_LIGHT_BLOCKS.get());
		else
			BrainUtils.setMemory(entity, AftershockMemoryTypes.NEARBY_LIGHT_BLOCKS.get(), blocks);
	}
}