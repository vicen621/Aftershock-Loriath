...
public class SoundPanic extends Behavior<PathfinderMob> {
	...
	public SoundPanic(float f) {
		this(f, DEFAULT_SHOULD_PANIC_PREDICATE);
	}

	public SoundPanic(float f, Predicate<PathfinderMob> predicate) {
	...
	protected boolean checkExtraStartConditions(ServerLevel serverLevel, PathfinderMob pathfinderMob) {
		return this.shouldPanic.test(pathfinderMob) && Explosion.getSeenPercent(new Vec3(pathfinderMob.getX(), pathfinderMob.getY(), pathfinderMob.getZ()), pathfinderMob) > 1;
	}
